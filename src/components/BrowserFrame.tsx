export default function BrowserFrame({ src, title }: { src: string; title: string }) {
  return (
    <div className="rounded-xl overflow-hidden shadow-2xl border border-white/10 bg-[#1a1a1a] w-full">
      <div className="flex items-center gap-2 px-4 py-3 bg-[#2a2a2a] border-b border-white/10">
        <span className="w-3 h-3 rounded-full bg-[#ff5f57]" />
        <span className="w-3 h-3 rounded-full bg-[#febc2e]" />
        <span className="w-3 h-3 rounded-full bg-[#28c840]" />
        <div className="flex-1 mx-3 bg-[#3a3a3a] rounded-md px-3 py-1 text-xs text-white/30 font-mono tracking-wide">
          {title}
        </div>
      </div>
